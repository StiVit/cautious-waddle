...
async def read_transaction(transaction_id: int, db: Session = Depends(get_db)):
    ...
    if db_transaction is None:
        endpoints_logger.info(f"Transaction {transaction_id} not found")
    else:
        endpoints_logger.info(f"Transaction {transaction_id} successfully found")
...
async def read_product(product_id: int, db: Session = Depends(get_db)):
    ...
    if db_product is None:
        endpoints_logger.info(f"Product {product_id} not found")
    else:
        endpoints_logger.info(f"Product {product_id} successfully found")
        return db_product
...
async def update_transaction(transaction_id: int, transaction: schemas.TransactionUpdate,
    ...
    if db_transaction is None:
        ...
        endpoints_logger.info(f"Transaction {transaction_id} successfully changed")
        return db_transaction
...
async def update_product(product_id: int, product: schemas.ProductUpdate, db: Session = Depends(get_db)):
    ...
    if db_product is None:
        ...
        endpoints_logger.info(f"Product {product_id} successfully changed")
        return db_product
...
async def delete_transaction(transaction_id: int, db: Session = Depends(get_db)):
    ...
    if db_transaction is None:
        ...
        endpoints_logger.info(f"Transaction {transaction_id} successfully deleted")
