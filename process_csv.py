...
def run_script():
    ...
    transactions['payment_method'] = transactions['payment_method'].apply(lambda x: PaymentMethod[x.replace(" ", "_").upper()])
    products['product_category'] = products['product_category'].apply(lambda x: ProductCategory[x.replace(" ", "_").upper()])

    # TODO: if data exists then ignore
